...
def epo_optimize(img, population_size, iterations, chaotic_sequence_length):
    ...
    with open(f"psnr_values_henon_{population_size}_{iterations}.txt", "w") as f:
...
def main():
    ...
    for i in range(3):
        for j in range(3):
            best_initial_values, best_psnr = epo_optimize(
                img,
                population_size=10*(i+1),
                iterations=50*(j+1),
                chaotic_sequence_length=chaotic_sequence_length,
            )
            print(
                f"Best initial values for Henon map: {best_initial_values}, Best psnr: {best_psnr}"
            )
